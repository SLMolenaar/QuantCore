...
public:
    ...
    double get_unrealized_pnl() const {
        ...
        return position * (mid.value() - avg_price);
    }

    double get_total_pnl() const { return realized_pnl_ + get_unrealized_pnl(); }
    ...
    Stats get_stats() const {
        ...
        s.orders_in_book = orderbook_.Size();
        return s;
    }
...
private:
    ...
    void update_position(const Trade& trade, bool is_taker) {
        ...
        if (were_buyer) {
            double fee = calculate_fee(effective_price, quantity, is_taker);
            total_fees_   += fee;
            realized_pnl_ -= fee;
            ...
            if (current_position < 0) {
                ...
                if (current_position == 0.0) {
                    ...
                    double total_cost    = current_position * current_avg_price
