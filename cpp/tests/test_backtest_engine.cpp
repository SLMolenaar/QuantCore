...
class SellSignalStrategy : public Strategy {
public:
    SellSignalStrategy() : Strategy("SellSignalStrategy"), triggered_(false) {}

    void on_data(const MarketDataEvent& event) override {
        if (!triggered_ && event.get_timestamp() == 1000000000LL) {
            generate_signal(event.get_symbol(), SignalType::SELL, 1.0, event.get_timestamp());
            triggered_ = true;
        }
    }
...
TEST_F(BacktestEngineTest, SellSignalCreatesShortPosition) {
    auto bars = create_flat_bars(10, 100.0);
    auto strategy = std::make_shared<SellSignalStrategy>();

    // Disable risk limits to allow short
    RiskLimits limits;
    limits.enabled = false;
    engine_->set_risk_limits(limits);
    ...
    // Should have short position (negative)
    EXPECT_LT(exec->get_position(), 0.0);
}